...
using ParallelCascades.CameraControllers.Runtime.UI;
using UnityEngine;

namespace ParallelCascades.CameraControllers.Runtime
{
    public class SceneLikeFlyCameraController : MonoBehaviour
    {
        [SerializeField] private float m_baseMovementSpeed = 1;
        [SerializeField] private float m_lookSensitivity = .3f;
        
        [SerializeField] private float m_buttonHeldSpeedUpMultiplier = 5;
        ...
        [SerializeField] private ToastDisplay m_toastDisplay;
        ...
        [SerializeField] private SingleCursorStateSO m_activeUseCursorState;

        private float m_movementSpeed;

        private Vector3 m_movementVelocity;
        /// <summary>
        /// As the camera moves, it accumulates speed during a single move, which 
        /// </summary>
        private float m_accumulatedMovementSpeedMultiplier;
        ...
        private float m_scrollableMovementSpeedMultiplier = 1f;
        private const float k_scrollableMovementSpeedMultiplierIncrement = 0.04f;

        private Quaternion m_targetPitchRotation;
        
        private SceneLikeFlyCameraControls m_cameraControls;
        ...
        private struct CameraInputs
        {
            ...
            public float AdjustMovementSpeed;
            public bool UseCameraIsHeld;
        }
        ...
        private CameraInputs GetCameraInputs()
        {
            CameraInputs cameraInputs = new CameraInputs
            {
                ...
                AdjustMovementSpeed = m_cameraControls.Camera.AdjustMovementSpeed.ReadValue<Vector2>().y, // note that in the new Input system scroll is Vector2, y is the actual scroll value
                UseCameraIsHeld = m_cameraControls.Camera.HoldToUse.IsPressed()
            };
            return cameraInputs;
        }
        ...
        private void Awake()
        {
            m_cameraControls = new SceneLikeFlyCameraControls();
        ...
        private void Update()
        {
            ...
            AdjustMovementSpeedThroughScroll(cameraInputs.AdjustMovementSpeed);
            ApplySpeedUpModifiers(cameraInputs.SpeedUpModifierPressed);
        ...
        private void AdjustMovementSpeedThroughScroll(float cameraInputsSpeedUpMove)
        {
            if (Mathf.Abs(cameraInputsSpeedUpMove) > 0.01f)
            {
                m_scrollableMovementSpeedMultiplier +=
                    cameraInputsSpeedUpMove * k_scrollableMovementSpeedMultiplierIncrement;
                m_scrollableMovementSpeedMultiplier = Mathf.Clamp(m_scrollableMovementSpeedMultiplier, 0.001f, 2f);

                if (m_toastDisplay != null)
                {
                    string stringFormat = m_scrollableMovementSpeedMultiplier != 0.001f ? "0.00" : "0.000";
                    m_toastDisplay.DisplayText($"{m_scrollableMovementSpeedMultiplier.ToString(stringFormat)}x");
                }
            }
        }
        ...
        {
            bool wasIgnoring = m_ignoreInput;
            m_ignoreInput = !cameraInputs.UseCameraIsHeld;

            if (m_activeCursorState != null && wasIgnoring != m_ignoreInput)
            {
                m_activeCursorState.SetCursorState(m_ignoreInput ? m_defaultCursorState : m_activeUseCursorState);
            }
        }
        ...
        private void ApplySpeedUpModifiers(bool speedUpModifierPressed)
        {
            m_movementSpeed = m_baseMovementSpeed * m_scrollableMovementSpeedMultiplier;
            if (speedUpModifierPressed)
            {
                m_movementSpeed *= m_buttonHeldSpeedUpMultiplier;
            }
        }
        ...
        private void CalculateCurrentMoveVelocity(Vector3 moveInput)
        {
            ...
            {
                // First frame of movement: begin speed at 1
                if (m_accumulatedMovementSpeedMultiplier == 0f)
                {
                    m_accumulatedMovementSpeedMultiplier = 1;
                }
                else
                {
                    m_accumulatedMovementSpeedMultiplier *= 1f + Time.deltaTime;
                }

                m_movementVelocity = Vector3.Lerp(m_movementVelocity,
                    worldMoveInputs * (m_accumulatedMovementSpeedMultiplier * m_movementSpeed),
                    SharpnessUtility.GetSharpnessInterpolant(m_movementSharpness, Time.deltaTime));
            }
            else
            {
                m_accumulatedMovementSpeedMultiplier = 0;
                m_movementVelocity = Vector3.Lerp(m_movementVelocity,
                    Vector3.zero,
                    SharpnessUtility.GetSharpnessInterpolant(m_movementSharpness, Time.deltaTime));
            }
